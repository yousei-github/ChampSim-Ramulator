    ...
    for (int r = 0; r < max_rank_count; r++)
    {
        ...
        for (int b = 0; b < max_bank_count; b++)
        {
            ...
            for (Request req : ctrl->otherq.q)
                if (req.type == Request::Type::REFRESH && req.addr_vec[level_chan] == ctrl->channel->id && req.addr_vec[level_rank] == r && req.addr_vec[level_bank] == bidx)
                    pending_ref = true;
            if (pending_ref)
                continue;
...
void Refresh<DSARP>::wrp()
{
    for (int ref_rid = 0; ref_rid < max_rank_count; ref_rid++)
    {
        ...
        if ((*(bank_refresh_backlog[ref_rid]))[ref_bid] < backlog_max && ctrl->otherq.q.size() < ctrl->otherq.max)
        {
