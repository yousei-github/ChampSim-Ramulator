...
class GenericDRAMController final : public IDRAMController, public Implementation {
  ...
  public:
    ...
    size_t get_queue_occupancy(const int type) const override
    {
        size_t occupancy = 0;

        if (type == Request::Type::Read)
        {
            occupancy = m_read_buffer.size();
        }
        else if (type == Request::Type::Write)
        {
            occupancy = m_write_buffer.size();
        }
        else
        {
            throw std::runtime_error("Invalid request type!");
    ...
    size_t get_queue_size(const int type) const override
    {
        size_t size = 0;

        if (type == Request::Type::Read)
        {
            size = m_read_buffer.max_size;
        }
        else if (type == Request::Type::Write)
        {
            size = m_write_buffer.max_size;
        }
        else
        {
            throw std::runtime_error("Invalid request type!");
