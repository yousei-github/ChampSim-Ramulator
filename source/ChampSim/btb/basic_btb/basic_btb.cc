...
void O3_CPU::btb_btbDbasic_btb_initialize_btb()
{
#if (PRINT_STATISTICS_INTO_FILE == ENABLE)
    fprintf(output_statistics.file_handler, "Basic BTB sets: %ld ways: %ld indirect buffer size: %ld RAS size: %ld\n", BTB_SET, BTB_WAY, BTB_INDIRECT_SIZE, RAS_SIZE);
#else
    std::cout << "Basic BTB sets: " << BTB_SET << " ways: " << BTB_WAY << " indirect buffer size: " << BTB_INDIRECT_SIZE << " RAS size: " << RAS_SIZE << std::endl;
#endif

    ::BTB.insert({
        this, champsim::msl::lru_table<btb_entry_t> {BTB_SET, BTB_WAY}
    });
...
std::pair<uint64_t, uint8_t> O3_CPU::btb_btbDbasic_btb_btb_prediction(uint64_t ip)
{
...
void O3_CPU::btb_btbDbasic_btb_update_btb(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type)
{
