...
#include "ChampSim/util/bits.h" // for lg2, bitmask
...
: champsim::operable(mc_period), address_mapping(addr_mapper), WQ {wq_size}, RQ {rq_size}, channel_width(width),
  ...
  DRAM_DBUS_BANKGROUP_STALL(std::chrono::duration_cast<champsim::chrono::clock::duration>((dbus_period * std::max(address_mapping.prefetch_size / 3, std::size_t {1})))),
    ...
    std::size_t banks, std::size_t columns, std::size_t ranks, std::size_t rows) -> slicer_type
{
    ...
    return std::apply([](auto... p)
        { return champsim::make_contiguous_extent_set(0, champsim::lg2(p)...); }, params);
}
...
void DRAM_CHANNEL::swap_write_mode()
{
    ...
    auto wq_occu                         = static_cast<std::size_t>(std::count_if(std::begin(WQ), std::end(WQ), [](const auto& x)
                                { return x.has_value(); }));
    auto rq_occu                         = static_cast<std::size_t>(std::count_if(std::begin(RQ), std::end(RQ), [](const auto& x)
                                { return x.has_value(); }));

    // Change modes if the queues are unbalanced
    if ((! write_mode && (wq_occu >= DRAM_WRITE_HIGH_WM || (rq_occu == 0 && wq_occu > 0))) || (write_mode && (wq_occu == 0 || (rq_occu > 0 && wq_occu < DRAM_WRITE_LOW_WM))))
    {
...
long DRAM_CHANNEL::populate_dbus()
{
    ...
    auto iter_next_process = std::min_element(std::begin(bank_request), std::end(bank_request),
        ...
        { return ! rhs.valid || (lhs.valid && lhs.ready_time < rhs.ready_time); });
...
void MEMORY_CONTROLLER::initialize()
{
    ...
    if (champsim::data::gibibytes gb_sz {sz}; gb_sz > 1_GiB)
    {
        std::fprintf(output_statistics.file_handler, "Off-chip DRAM Size: %lld", gb_sz);
    }
    else if (champsim::data::mebibytes mb_sz {sz}; mb_sz > 1_MiB)
    {
        std::fprintf(output_statistics.file_handler, "Off-chip DRAM Size: %lld", mb_sz);
    }
    else if (champsim::data::kibibytes kb_sz {sz}; kb_sz > 1_kiB)
    {
        std::fprintf(output_statistics.file_handler, "Off-chip DRAM Size: %lld", kb_sz);
    }
    else
    {
        std::fprintf(output_statistics.file_handler, "Off-chip DRAM Size: %lld", sz);
    }
    std::fprintf(output_statistics.file_handler, " Channels: %ld Width: %ld-bit Data Rate: %ld MT/s\n", std::size(channels), champsim::data::bits_per_byte * channel_width.count(), 1us / (data_bus_period));
...
bool MEMORY_CONTROLLER::add_rq(const request_type& packet, champsim::channel* ul)
{
    ...
    if (auto rq_it = std::find_if_not(std::begin(channel.RQ), std::end(channel.RQ), [this](const auto& pkt)
