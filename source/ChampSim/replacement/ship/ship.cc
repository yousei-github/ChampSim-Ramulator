...
void CACHE::repl_replacementDship_initialize_replacement()
{
...
uint32_t CACHE::repl_replacementDship_find_victim(uint32_t triggering_cpu, uint64_t instr_id, uint32_t set, const BLOCK* current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
...
void CACHE::repl_replacementDship_update_replacement_state(uint32_t triggering_cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    ...
    if (s_idx != std::end(::rand_sets[this]))
    {
        ...
        auto match       = std::find_if(s_set_begin, s_set_end,
                  [addr = full_addr, shamt = 8 + champsim::lg2(NUM_WAY)](auto x)
...
void CACHE::repl_replacementDship_replacement_final_stats() {}
