...
void O3_CPU::bpred_branchDhashed_perceptron_initialize_branch_predictor()
{
#if (PRINT_STATISTICS_INTO_FILE == ENABLE)
    fprintf(output_statistics.file_handler, "CPU %d Hashed Perceptron branch predictor\n", cpu);
#else
    std::cout << "CPU " << cpu << " Hashed Perceptron branch predictor" << std::endl;
#endif
...
uint8_t O3_CPU::bpred_branchDhashed_perceptron_predict_branch(uint64_t pc)
{
...
void O3_CPU::bpred_branchDhashed_perceptron_last_branch_result(uint64_t pc, uint64_t branch_target, uint8_t taken, uint8_t branch_type)
{
