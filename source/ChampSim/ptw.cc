...
#include "ChampSim/ptw_builder.h" // for ptw_builder
#include "ChampSim/util/bits.h"   // for bitmask, lg2, splice_bits
  ...
  HIT_LATENCY(b.m_clock_period * b.m_latency), vmem(b.m_vmem), CR3_addr(b.m_vmem->get_pte_pa(b.m_cpu, champsim::page_number {}, b.m_vmem->pt_levels).first)
{
    ...
    std::remove_copy_if(std::begin(b.m_pscl), std::end(b.m_pscl), std::back_inserter(local_pscl_dims), [](auto x)
...
auto PageTableWalker::handle_read(const request_type& handle_pkt, channel_type* ul) -> std::optional<mshr_type>
{
    ...
    std::transform(std::begin(pscl), std::end(pscl), std::back_inserter(pscl_hits), [walk_init](auto& x)
        { return x.check_hit(walk_init); });
    walk_init =
        std::accumulate(std::begin(pscl_hits), std::end(pscl_hits), std::optional<pscl_entry>(walk_init), [](auto x, auto& y)
            { return y.value_or(*x); })
            .value();
    ...
    if constexpr (champsim::debug_print)
    {
        ...
        fmt::print("[{}] {} address: {} v_address: {} pt_page_offset: {} translation_level: {} cycle: {}\n",
            NAME, __func__, fwd_mshr.address, handle_pkt.v_address, walk_offset.to<int>(), walk_init.level, current_time.time_since_epoch() / clock_period);
#endif /* USE_VCPKG */
    }
...
auto PageTableWalker::step_translation(const mshr_type& source) -> std::optional<mshr_type>
{
    ...
    packet.type_origin = access_type::TRANSLATION;
...
long PageTableWalker::operate()
{
    ...
    if constexpr (champsim::debug_print)
    {
        if (progress > 0)
        {
            ...
            std::transform(std::begin(MSHR), std::end(MSHR), std::back_inserter(mshr_addresses), [](const auto& x)
                { return x.address; });
