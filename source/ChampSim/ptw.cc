...
long PageTableWalker::operate()
{
    ...
    auto [mshr_begin, mshr_end] =
        champsim::get_span_p(std::cbegin(finished), std::cend(finished), fill_bw, [cycle = current_cycle](const auto& pkt)
            { return pkt.event_cycle <= cycle; });
...
void PageTableWalker::finish_packet(const response_type& packet)
{
    ...
    auto last_finished =
        std::partition(std::begin(MSHR), std::end(MSHR), [addr = packet.address](auto x)
