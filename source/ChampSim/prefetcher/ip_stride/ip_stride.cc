...
void CACHE::pref_prefetcherDip_stride_prefetcher_initialize()
{
#if (PRINT_STATISTICS_INTO_FILE == ENABLE)
    fprintf(output_statistics.file_handler, "%s IP-based stride prefetcher\n", NAME.c_str());
#else
    std::cout << NAME << " IP-based stride prefetcher" << std::endl;
#endif
}

void CACHE::pref_prefetcherDip_stride_prefetcher_cycle_operate()
{
    ::trackers[this].advance_lookahead(this);
}

uint32_t CACHE::pref_prefetcherDip_stride_prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in)
{
    ::trackers[this].initiate_lookahead(ip, addr >> LOG2_BLOCK_SIZE);
    return metadata_in;
}

uint32_t CACHE::pref_prefetcherDip_stride_prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
    return metadata_in;
}

void CACHE::pref_prefetcherDip_stride_prefetcher_final_stats() {}
