...
template<class T>
MEMORY_CONTROLLER<T>::MEMORY_CONTROLLER(double freq_scale, double clock_scale, std::vector<channel_type*>&& ul, Memory<T, Controller>& memory)
...
bool MEMORY_CONTROLLER<T>::add_rq(const request_type& packet, champsim::channel* ul)
{
    ...
    DRAM_CHANNEL::request_type rq_it = request_type {packet};
    ...
    if (address < memory.max_address)
    {
        Request request(address, Request::Type::READ, std::bind(&MEMORY_CONTROLLER<T>::return_data, this, placeholders::_1), rq_it, packet.cpu, memory_id);
        stall = ! memory.send(request);
...
bool MEMORY_CONTROLLER<T>::add_wq(const request_type& packet)
{
    ...
    if (address < memory.max_address)
    {
        Request request(address, Request::Type::WRITE, std::bind(&MEMORY_CONTROLLER<T>::return_data, this, placeholders::_1), wq_it, packet.cpu, memory_id);
        stall = ! memory.send(request);
