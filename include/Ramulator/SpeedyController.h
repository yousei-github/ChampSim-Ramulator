...
public:
    ...
    void tick()
    {
        ...
        request_queue& q = otherq.size() ? otherq : write_mode ? writeq :
                                                                 readq;
...
private:
    ...
    void update(typename T::Command cmd, bool state_change, vector<int>::iterator& begin, vector<int>::iterator& end, request_queue& q)
    {
        ...
        for (auto& info : q)
        {
            ...
            if ((cmd == T::Command::RD || cmd == T::Command::WR) && get<1>(info) == T::Command::ACT)
                continue;
    ...
    void schedule(request_queue& q)
    {
        ...
        bool state_change = channel->spec->is_opening(first_cmd) || channel->spec->is_closing(first_cmd) || channel->spec->is_refreshing(first_cmd);
