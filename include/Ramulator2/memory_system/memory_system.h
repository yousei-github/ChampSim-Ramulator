...
class IMemorySystem : public TopLevel<IMemorySystem> {
  ...
  public:
    ...
    virtual int get_channel() const { return 0; };

    virtual int get_channel_width() const { return 0; };

    virtual int get_rate() const { return 0; };

    virtual void set_clock_ratio(uint clock_ratio) { m_clock_ratio = clock_ratio; };

    /**
     * @brief     Get the number of current members of related queue
     * @param[in] req The memory request
     * @return    Number of queue's current members
     */
    virtual size_t get_queue_occupancy(Request& req) const { return 0; };
     ...
     * @return    Capacity
     */
    virtual size_t get_queue_size(Request& req) const { return 0; };
#endif /* USER_CODES */
};
