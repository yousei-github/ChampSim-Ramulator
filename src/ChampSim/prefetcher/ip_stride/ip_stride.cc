...
void CACHE::pref_pprefetcherDip_stride_initialize() { std::cout << NAME << " IP-based stride prefetcher" << std::endl; }

void CACHE::pref_pprefetcherDip_stride_cycle_operate()
{
...
uint32_t CACHE::pref_pprefetcherDip_stride_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
...
uint32_t CACHE::pref_pprefetcherDip_stride_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
  return metadata_in;
}

void CACHE::pref_pprefetcherDip_stride_final_stats() {}
#else
constexpr int PREFETCH_DEGREE = 3;
