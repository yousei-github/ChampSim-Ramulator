...
void O3_CPU::bpred_bbranchDgshare_initialize()
{
...
uint8_t O3_CPU::bpred_bbranchDgshare_predict(uint64_t ip, uint64_t predicted_target, uint8_t always_taken, uint8_t branch_type)
{
  int prediction = 1;
...
void O3_CPU::bpred_bbranchDgshare_last_result(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type)
{
  int gs_hash = gs_table_hash(ip, branch_history_vector[cpu]);
