...
// sampler
std::map<CACHE*, std::vector<std::size_t>> rand_sets_for_ship;
...
void CACHE::repl_rreplacementDship_initialize()
{
  ...
  for (std::size_t i = 0; i < SAMPLER_SET; i++)
  {
    ...
    std::vector<std::size_t>::iterator loc = std::lower_bound(std::begin(rand_sets_for_ship[this]), std::end(rand_sets_for_ship[this]), val);

    while (loc != std::end(rand_sets_for_ship[this]) && *loc == val)
    {
      rand_seed = rand_seed * 1103515245 + 12345;
      val = (rand_seed / 65536) % NUM_SET;
      loc = std::lower_bound(std::begin(rand_sets_for_ship[this]), std::end(rand_sets_for_ship[this]), val);
    }

    rand_sets_for_ship[this].insert(loc, val);
  }
...
uint32_t CACHE::repl_rreplacementDship_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK* current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
...
void CACHE::repl_rreplacementDship_update(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type,
                                          uint8_t hit)
{
  ...
  // update sampler
  auto s_idx = std::find(std::begin(rand_sets_for_ship[this]), std::end(rand_sets_for_ship[this]), set);
  if (s_idx != std::end(rand_sets_for_ship[this]))
  {
    auto s_set_begin = std::next(std::begin(sampler[this]), std::distance(std::begin(rand_sets_for_ship[this]), s_idx));
...
void CACHE::repl_rreplacementDship_final_stats() {}
#else
#define maxRRPV 3
#define SHCT_SIZE 16384
#define SHCT_PRIME 16381
