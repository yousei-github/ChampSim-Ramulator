...
std::map<CACHE*, std::vector<std::size_t>> rand_sets_for_drrip;
std::map<std::pair<CACHE*, std::size_t>, unsigned> PSEL;

void CACHE::repl_rreplacementDdrrip_initialize()
{
  ...
  for (std::size_t i = 0; i < TOTAL_SDM_SETS; i++)
  {
    ...
    auto loc = std::lower_bound(std::begin(rand_sets_for_drrip[this]), std::end(rand_sets_for_drrip[this]), val);

    while (loc != std::end(rand_sets_for_drrip[this]) && *loc == val)
    {
      rand_seed = rand_seed * 1103515245 + 12345;
      val = (rand_seed / 65536) % NUM_SET;
      loc = std::lower_bound(std::begin(rand_sets_for_drrip[this]), std::end(rand_sets_for_drrip[this]), val);
    }

    rand_sets_for_drrip[this].insert(loc, val);
  }
}

// called on every cache hit and cache fill
void CACHE::repl_rreplacementDdrrip_update(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
  ...
  // cache miss
  auto begin = std::next(std::begin(rand_sets_for_drrip[this]), cpu * NUM_POLICY * SDM_SIZE);
...
uint32_t CACHE::repl_rreplacementDdrrip_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK* current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
...
void CACHE::repl_rreplacementDdrrip_final_stats() {}
#else
#define maxRRPV 3
#define NUM_POLICY 2
#define SDM_SIZE 32
